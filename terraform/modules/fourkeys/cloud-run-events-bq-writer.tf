...
module "gcloud_build_batch_events_bq_writer" {
  ...
  create_cmd_body        = "builds submit ../batch_events --tag=${local.events_bq_writer_container_url}:${data.archive_file.events_bq_writer_source.output_sha} --project=${var.project_id} --gcs-log-dir=gs://tf-cloud-build-logs"
