...
module "gcloud_build_batch_events_bq_writer" {
  ...
  create_cmd_body        = "builds submit --tag=${local.events_bq_writer_container_url}:${data.archive_file.events_bq_writer_source.output_sha} --project=${var.project_id} --no-source --gcs-log-dir=gs://tf-cloud-build-logs"
