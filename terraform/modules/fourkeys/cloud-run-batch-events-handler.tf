...
module "gcloud_build_batch_events_handler" {
  ...
  create_cmd_body        = "builds submit ../batch_events_handler --tag=${local.batch_events_handler_container_url}:${data.archive_file.batch_events_handler_source.output_sha} --project=${var.project_id}"
...
resource "google_cloud_run_service_iam_binding" "batch_event_handler_noauth" {
  location   = var.region
  project    = var.project_id
  service    = google_cloud_run_service.batch_events_handler.name
  role       = "roles/run.invoker"
  members    = ["allUsers"]
  depends_on = [google_cloud_run_service.batch_events_handler]
}
