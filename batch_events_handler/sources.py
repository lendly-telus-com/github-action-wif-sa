...
def get_source(headers):
    ...
    if "X-Apps-Scripts" in headers:
        return headers.get("X-Apps-Scripts", "")

    if headers.get("User-Agent") == "ruxit server":
        return "dynatrace"
...
AUTHORIZED_SOURCES = {
    "GitHub-Hookshot": EventSource("X-Hub-Signature", github_verification),
    ...
    "spreadsheets": EventSource("Authorization", simple_token_verification),
    "dynatrace": EventSource("Authorization", simple_token_verification),
}
